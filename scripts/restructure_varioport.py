    ...
    if os.path.isdir(participants_tsv):
        ...
        for fn in os.listdir(participants_tsv):
            if fn.lower() == 'participants.tsv':
                ...
                break
        if not found:
            raise ValueError(f"No participants.tsv found in directory: {participants_tsv}")
...
def main():
    p = argparse.ArgumentParser(description='Restructure Varioport RAW files into sourcedata folders')
    ...
    p.add_argument('--dst', required=True, help='Destination root (where sourcedata/ will be created)')
    p.add_argument('--recursive', action='store_true', help='Scan source folder recursively')
    p.add_argument('--dry-run', action='store_true', help='Show actions but do not copy files')
    args = p.parse_args()
    ...
    print(f'Loaded {sum(len(v) for v in suffix_map.values())} participants across {len(suffix_map)} suffix keys')
    ...
    with open(map_log_path, 'w', newline='', encoding='utf-8') as mapf, \
         open(ambiguous_log, 'w', newline='', encoding='utf-8') as ambf:
        map_writer = csv.writer(mapf)
        amb_writer = csv.writer(ambf)
        map_writer.writerow(['src_path', 'basename', 'suffix', 'participant_id', 'session', 'dst_path', 'note'])
        ...
        for fp in files:
            ...
            if not suffix:
                print(f'Warning: no 3-digit suffix found for {bn} in path {fp}; skipping')
                map_writer.writerow([fp, bn, '', '', '', '', 'no-suffix'])
                continue

            candidates = suffix_map.get(suffix, [])
            if not candidates:
                print(f'No participant matches suffix {suffix} for file {bn}; logging as unmatched')
