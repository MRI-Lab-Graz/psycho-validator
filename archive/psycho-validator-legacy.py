...
def _print_bids_schema_summary(local_path=None):
    ...
    if not schema:
        ...
        print(
            "💡 Install one of: pip install bidsschematools | pip install bids-schema (if available)"
        )
    ...
    if isinstance(schema, dict):
        ...
        datatypes = schema.get("datatypes") or schema.get("objects", {}).get(
            "datatypes"
        )
    ...
    if entities:
        names = (
            ...
            else list(map(str, entities))
        )
        print(
            f"🔹 Entities ({len(names)}): {', '.join(sorted(names)[:20])}{'...' if len(names)>20 else ''}"
        )
    if datatypes:
        names = (
            list(datatypes.keys())
            if isinstance(datatypes, dict)
            else list(map(str, datatypes))
        )
    ...
    if suffixes:
        names = (
            ...
            else list(map(str, suffixes))
        )
        # Show common MRI suffixes if present
        show = [
            n
            for n in names
            if n in ("T1w", "T2w", "bold", "dwi", "phasediff", "fieldmap", "epi")
        ]
        ...
        for line_num, line in enumerate(lines[1:], 2):
            if line.strip():  # Skip empty lines
                ...
                if len(values) > participant_id_idx:
                    ...
                    issues.append(
                        f"participants.tsv line {line_num}: insufficient columns"
                    )
...
def validate_participants_consistency(stats, participants_info):
    ...
    for subject in missing_in_participants:
        issues.append(
            (
                "WARNING",
                f"Subject {subject} found in data but not listed in participants.tsv",
            )
        )
    ...
    for participant in missing_in_data:
        issues.append(
            (
                "WARNING",
                f"Participant {participant} listed in participants.tsv but no data found",
            )
        )
    ...
    def check_consistency(self):
        ...
        if subjects_with_sessions and subjects_without_sessions:
            warnings.append(
                (
                    "WARNING",
                    f"Mixed session structure: {len(subjects_with_sessions)} subjects have sessions, {len(subjects_without_sessions)} don't",
                )
            )
        ...
        for session in all_sessions:
            ...
            for subject_id in subjects_with_this_session:
                if session in subjects_with_sessions[subject_id]["session_data"]:
                    subject_session_data = subjects_with_sessions[subject_id][
                        "session_data"
                    ][session]

                    missing_modalities = (
                        session_modalities - subject_session_data["modalities"]
                    )
                    ...
                    for modality in missing_modalities:
                        warnings.append(
                            (
                                "WARNING",
                                f"Subject {subject_id} session {session} missing {modality} data",
                            )
                        )
                    ...
                    for task in missing_tasks:
                        warnings.append(
                            (
                                "WARNING",
                                f"Subject {subject_id} session {session} missing task {task}",
                            )
                        )
                ...
                if isinstance(level_item.get("files"), list):
                    for fi in level_item["files"]:
                        ...
                        if isinstance(fi, dict):
                            if isinstance(fi.get("file"), dict) and fi["file"].get(
                                "path"
                            ):
                                paths.append(fi["file"]["path"])
                            elif fi.get("path"):
                                paths.append(fi["path"])
                if isinstance(level_item.get("file"), dict) and level_item["file"].get(
                    "path"
                ):
...
def validate_dataset(root_dir, bids_mode="auto"):
    ...
    def _ensure_bidsignore(base_dir):
        ...
        try:
            ...
            issues.append(
                (
                    "INFO",
                    "Created .bidsignore so bids-validator ignores custom modalities",
                )
            )
            return True
        except Exception as e:
    ...
    if bids_mode != "off":
        ...
        if bi is not None:
            ...
            # Add an info line
            issues.append(
                ("INFO", "Using official BIDS validator for core BIDS checks")
            )
        elif bids_mode == "auto":
            issues.append(
                ("INFO", "bids-validator not found; falling back to internal checks")
            )
    ...
    for item in os.listdir(root_dir):
        ...
        if os.path.isdir(item_path) and item.startswith("sub-"):
            ...
            subject_issues = validate_subject(
                item_path, item, stats, root_dir, skip_core_bids_checks
            )
    ...
    participant_consistency_issues = validate_participants_consistency(
        stats, participants_info
    )
...
def validate_subject(
    subject_dir, subject_id, stats, root_dir, skip_core_bids_checks=False
):
        ...
        if os.path.isdir(item_path):
            if item.startswith("ses-"):
                ...
                issues += validate_session(
                    item_path, subject_id, item, stats, root_dir, skip_core_bids_checks
                )
            elif item == "mri":
                # MRI container without sessions
                if not skip_core_bids_checks:
                    issues += validate_mri_container(
                        item_path, subject_id, None, stats, root_dir
                    )
                else:
                    # Stats-only scan of MRI container
                    for sub in os.listdir(item_path):
                        sub_path = os.path.join(item_path, sub)
                        if os.path.isdir(sub_path) and sub in (
                            "anat",
                            "func",
                            "fmap",
                            "dwi",
                ...
                if skip_core_bids_checks and item in ("anat", "func", "fmap", "dwi"):
                    ...
                    issues += validate_modality_dir(
                        item_path, subject_id, None, item, stats, root_dir
                    )
...
def validate_session(
    session_dir, subject_id, session_id, stats, root_dir, skip_core_bids_checks=False
):
    ...
    for item in os.listdir(session_dir):
        ...
        if os.path.isdir(item_path):
            if item == "mri":
                if not skip_core_bids_checks:
                    issues += validate_mri_container(
                        item_path, subject_id, session_id, stats, root_dir
                    )
                else:
                    # Stats-only scan of MRI container
                    for sub in os.listdir(item_path):
                ...
                if skip_core_bids_checks and item in ("anat", "func", "fmap", "dwi"):
                    ...
                    issues += validate_modality_dir(
                        item_path, subject_id, session_id, item, stats, root_dir
                    )
...
def validate_mri_container(mri_dir, subject_id, session_id, stats, root_dir):
    ...
    for sub in os.listdir(mri_dir):
        ...
        if os.path.isdir(sub_path) and sub in ("anat", "func", "fmap", "dwi"):
            issues += validate_modality_dir(
                sub_path, subject_id, session_id, sub, stats, root_dir
            )
...
def validate_modality_dir(
    modality_dir, subject_id, session_id, modality, stats, root_dir
):
    ...
    for fname in os.listdir(modality_dir):
        ...
        if os.path.isfile(file_path):
                ...
                if task_match:
                    ...
                    if re.search(r"[^A-Za-z0-9]", task):
                        ...
                        issues.append(
                            (
                                "ERROR",
                                f"{file_path}: invalid task label '{task}'. Use only letters/digits. Suggested: '{suggestion}'",
                            )
                        )
            ...
            if re.search(r"_stim\.", fname):
                ...
                if hint:
                    issues.append(
                        (
                            "ERROR",
                            f"{file_path}: non-BIDS suffix '_stim'. Use '_{hint}' instead and provide a matching sidecar JSON.",
                        )
                    )
                else:
                    issues.append(
                        (
                            "ERROR",
                            f"{file_path}: non-BIDS suffix '_stim'. Replace with a BIDS-appropriate suffix for this modality.",
                        )
                    )
                ...
                if is_nifti:
                    if not BIDS_REGEX.match(base) or not MRI_SUFFIX_REGEX.search(base):
                        issues.append(
                            (
                                "ERROR",
                                f"Invalid MRI filename: {fname} in {modality_dir}",
                            )
                        )
                else:
                    ...
                    if not BIDS_REGEX.match(base):
                        issues.append(
                            ("ERROR", f"Invalid filename: {fname} in {modality_dir}")
                        )
            else:
                if not BIDS_REGEX.match(base):
                    issues.append(
            ...
            if modality == "func":
                ...
                if re.match(r".+_events\.tsv(\.gz)?$", fname) or fname.endswith(
                    "_events.json"
                ):
                    allowed_extra = True
            if modality == "dwi":
            ...
            if ext == ".json":
                ...
                if modality == "physiological" and fname.endswith(".csv"):
                    issues.append(
                        (
                            "ERROR",
                            f"{file_path}: .csv not valid for BIDS physio. Use .tsv(.gz) with '_physio' suffix and a JSON data dictionary.",
                        )
                    )
                else:
                    issues.append(
                        (
                            "WARNING",
                            f"{file_path}: does not match expected pattern for modality '{modality}'",
                        )
                    )
            ...
            if subject_id not in fname:
                issues.append(
                    (
                        "ERROR",
                        f"Filename {fname} doesn't contain subject ID {subject_id}",
                    )
                )
            if session_id and session_id not in fname:
                issues.append(
                    (
                        "ERROR",
                        f"Filename {fname} doesn't contain session ID {session_id}",
                    )
                )
            ...
            if require_sidecar:
                ...
                if os.path.exists(sidecar):
                    ...
                    issues.append(
                        ("WARNING", f"Missing recommended data dictionary: {sidecar}")
                    )
        ...
        for prop, details in properties.items():
            ...
            if "enum" in details:
                ...
                if len(enum_values) <= 5:
                    ...
                    print(
                        f"{indent}  Options: {', '.join(map(str, enum_values[:3]))}... ({len(enum_values)} total)"
                    )
            ...
            if prop_type == "object" and "properties" in details:
                ...
                print_properties(
                    details["properties"], nested_required, indent_level + 1
                )
...
def create_fair_template(output_file):
    ...
    template = {
        ...
        "ResearchDomains": ["cognitive psychology", "experimental psychology"],
...
def generate_full_fair_report(dataset_root):
    ...
    if os.path.exists(dataset_desc_path):
        ...
        print(
            "❌ No dataset_description.json found - create one for better FAIR compliance"
        )
    ...
    if fair_scores:
        ...
        if overall_score >= 80:
            ...
            print(
                "🥉 FAIR - Basic compliance achieved, significant improvements needed"
            )
        else:
    ...
    if os.path.exists(dataset_desc_path):
        ...
        try:
            ...
            indicators = {
                ...
                "License": dataset_desc.get("License")
                not in [None, "", "All rights reserved"],
        ...
        print(
            "💡 Run: python psycho-validator.py --create-fair-template dataset_description.json"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--check-compatibility",
        nargs=2,
        metavar=("SCHEMA_VERSION", "REQUIRED_VERSION"),
    ...
    if args.check_compatibility:
        ...
        print(
            f"Schema version {schema_version} {'IS' if is_compat else 'IS NOT'} compatible with required version {required_version}"
        )
    ...
    if args.init_bidsignore:
        ...
        if os.path.exists(bidsignore_path) and not args.force_bidsignore:
            print(
                f"⚠️  .bidsignore already exists at {bidsignore_path} (use --force-bidsignore to overwrite)"
            )
            sys.exit(0)
        ...
        try:
            ...
            print(
                "💡 The official bids-validator will now ignore these paths; psycho-validator still checks them."
            )
        except Exception as e:
    ...
    if args.verbose:
        ...
        print(
            f"📋 Available schemas: {[k for k, v in SCHEMAS.items() if v is not None]}"
        )
