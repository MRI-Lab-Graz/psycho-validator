...
def export_dublin_core(metadata_file, output_file=None):
    ...
    if is_dataset_description:
        ...
        dc_mapping = {
            ...
            "dc:description": metadata.get(
                "Description", "Psychological research dataset"
            ),
            "dc:date": get_nested_value(
                metadata,
                "DataCollection.start_date",
                datetime.now().strftime("%Y-%m-%d"),
            ),
            "dc:type": "Dataset",
            "dc:identifier": metadata.get(
                "DatasetDOI", f"local:{os.path.basename(metadata_file)}"
            ),
            "dc:language": "en",
        ...
        dc_mapping = {
            ...
            "dc:creator": get_nested_value(
                metadata, "Metadata.Creator", "Unknown Creator"
            ),
            "dc:subject": get_nested_value(
                metadata, "Categories.PrimaryCategory", "Psychology"
            ),
            "dc:description": get_nested_value(
                metadata, "Metadata.Description", "Psychological stimulus data"
            ),
            "dc:date": get_nested_value(
                metadata, "Metadata.CreationDate", datetime.now().strftime("%Y-%m-%d")
            ),
            ...
            "dc:identifier": get_nested_value(
                metadata,
                "Metadata.DatasetDOI",
                f"local:{os.path.basename(metadata_file)}",
            ),
            "dc:language": "en",
            "dc:rights": get_nested_value(
                metadata, "Metadata.License", "All rights reserved"
            ),
        }
...
def export_datacite(metadata_file, output_file=None):
    ...
    if is_dataset_description:
        ...
        creator = ET.SubElement(creators, "creator")
        creator_name = ET.SubElement(creator, "creatorName")
        creator_name.text = get_nested_value(
            metadata, "Metadata.Creator", "Unknown Creator"
        )
    ...
    title.text = get_nested_value(
        metadata, "Study.TaskName", "Psychological Stimulus Dataset"
    )

    # Publisher
    publisher = ET.SubElement(root, "publisher")
    publisher.text = get_nested_value(
        metadata, "Metadata.Institution", "Unknown Institution"
    )

    # Publication Year
    pub_year = ET.SubElement(root, "publicationYear")
    creation_date = get_nested_value(
        metadata, "Metadata.CreationDate", datetime.now().strftime("%Y-%m-%d")
    )
    ...
    resource_type.text = (
        f"Psychology {get_nested_value(metadata, 'Technical.StimulusType', 'Data')}"
    )
    ...
    date_created.text = get_nested_value(
        metadata, "Metadata.CreationDate", datetime.now().strftime("%Y-%m-%d")
    )
    ...
    if license_info.startswith("CC"):
        rights.set(
            "rightsURI",
            f"https://creativecommons.org/licenses/{license_info.lower().replace('cc-', '')}/4.0/",
        )
